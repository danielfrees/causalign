...
def get_default_sent_training_args(regime: str):
    ...
    if 'sent' in regime: 
        ...
        parser.add_argument("--unfreeze_backbone_sentiment", type=str, default="all", help="Unfreeze the top{n} layers of the sentiment backbone model. Iterative unfreezes deeper throughout training. Options: 'top{n}', 'all', 'iterative'")
        parser.add_argument("--unfreeze_backbone_riesz", type=str, default="all", help="Unfreeze the top{n} layers of the riesz backbone model. Iterative unfreezes deeper throughout training. Options: 'top{n}', 'all', 'iterative'")
        ...
        if regime == 'causal_sent':
            ...
            # for riesz-only training, interleaved training (riesz, sentiment alternate training/ being frozen for stability)
            parser.add_argument("--riesz_only", action='store_true', default = False, help = "Whether to train only the Riesz head and freeze the rest of the model.")
            parser.add_argument("--interleaved_training", action='store_true', default = False, help = "Whether to train the Riesz head and sentiment head alternately, starting with the Riesz head.")
...
def process_unfreeze_param(unfreeze_param: str):
    """
    Process the unfreeze parameter for the backbone models.
    """
